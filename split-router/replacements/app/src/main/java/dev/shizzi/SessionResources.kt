...
class SessionResources(
        ...
        runCatching { connectivityManager.requestNetwork(request, callback) }
            .onFailure { failure ->
                ...
                throw IllegalStateException(
                    "requestKeepAlive: could not request the test network, it would be lingered away within seconds",
                    failure,
                )
            }
    }
    ...
    fun startDatapath(mtu: Int) {
        ...
        datapathSession = runCatching { Datapath.start(descriptor.fd.toLong(), mtu.toLong()) }
            .getOrElse { failure ->
                throw IllegalStateException(
                    "startDatapath: userspace stack failed to attach to fd ${descriptor.fd} (mtu=$mtu)",
                    failure,
                )
            }
    }
    ...
    fun setPhysicalNetworks(wifiHandle: Long, cellularHandle: Long) {
        val session = datapathSession
            ?: error("setPhysicalNetworks: no datapath session; startDatapath must succeed first")
        session.setNetworks(wifiHandle, cellularHandle)
    }
