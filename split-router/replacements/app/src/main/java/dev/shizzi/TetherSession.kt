...
class TetherSession(private val context: Context) {
    ...
    private var vpnMode = VpnMode.NEVER
    ...
    private val physical = PhysicalUpstreams(context)
    ...
    fun start(mode: VpnMode = VpnMode.AUTO): String {
        ...
        vpnMode = VpnMode.NEVER
        state = SessionState.STARTING
        SessionLog.info("split-router start requested (ignoring vpn mode ${mode.name.lowercase()})")
    ...
    private fun bringUp(): String {
        ...
        val handles = physical.acquire()
        group.setPhysicalNetworks(handles.wifi, handles.cellular)
        SessionLog.info(
            "physical upstreams ready: wifi=${handles.wifi}, cellular=${handles.cellular}; " +
                "policy X -> cellular, default -> wifi",
        )
        ...
        verifyUpstream(name)
        SessionLog.info("upstream verified: $name is sole tethering upstream")

        state = SessionState.ACTIVE
        activeSince = System.currentTimeMillis()
        detail = "Split routing active: X via cellular, everything else via Wi-Fi"
